...
exports.run = (client, message, args) => {
    let kayıt = db.get(`kayıt.${message.guild.id}.durum`)
        if (kayıt) {
    let kadın = db.get(`kayıt.${message.guild.id}.kadın.rol`)
    ...
    if(!kadın) return message.replyNoMention(`> **Kadın rolü** henüz ayarlanmamış. Tüm sistemleri ayarlayıp tekrar deneyin.`)
    if(!kayıtsız) return message.replyNoMention(`> **Kayıtsız rolü** henüz ayarlanmamış. Tüm sistemleri ayarlayıp tekrar deneyin.`)
      ...
      if(user.bot) return message.replyNoMention(`> Yalnızca **insanları** kayıt edebilirim.`)
          ...
          if(user.roles.cache.get(kadın)) return message.replyNoMention("> Kullanıcı zaten kayıtlı.")
        user.roles.add(kadın)
        user.roles.remove(kayıtsız)
       const embed = new Discord.MessageEmbed()
       .setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
       .setDescription(`${user} adlı kullanıcı, ${message.author} tarafından **kadın** olarak kayıt edildi.\n\n> Verilen Rol: <@&${kadın}>\n\n> Alınan Rol: <@&${kayıtsız}>`)
