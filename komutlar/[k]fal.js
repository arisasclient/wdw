...
if(await data.fetch(`slm.${message.author.id}.${message.guild.id}.fal`)) return message.replyNoMention(new Discord.MessageEmbed().setColor('#00001').setDescription(`**10 saniyenin altında hızlı kullanamazsın!**`));
...
message.replyNoMention(new Discord.MessageEmbed().setTitle(`${message.author.username} Bahtına bakalım 🤓`)
.setDescription(resimler).setColor('#cd0e63'));

data.set(`slm.${message.author.id}.${message.guild.id}.fal`, 'Firex');
setTimeout(() => {
